...
class ResNet_14_Manual():
    def __init__(self, model, N_SUBSPACE, K_CLUSTER, record_res_bool=True):
        ...
        self.layer5_weights = self.get_param(model.conv5_x)  
        self.layer5_bn_buffers = self.get_param(model.conv5_x, 'buffer')  
    ...
    def forward_exact_bn_fold(self, input_data):
        ...
        layer5 = self.forward_block_exact(layer4, self.layer5_weights, self.layer5_bn_buffers, stride=2)

        output = self.avg_pool(torch.tensor(layer5)).view(input_data.shape[0], -1).detach().numpy()  # torch.Size([3025, 16, 1, 1])
        ...
        layer_res = [layer1, layer2, layer3, layer4, layer5, output]
        mm_res = self.mm_res.copy()
        self.mm_res.clear()
        return layer_res, mm_res
    
    def train_amm(self, input_data, mask):
        if mask[0]:
        ...
        if mask[4]: 
            layer5 = self.train_block_amm(layer4, self.layer5_weights, self.layer5_bn_buffers, stride=2, n=self.n_subspace[4], k=self.k_cluster[4])
        else:
            layer5 = self.forward_block_exact(layer4, self.layer5_weights, self.layer5_bn_buffers, stride=2)
            
        output = self.avg_pool(torch.tensor(layer5)).view(input_data.shape[0], -1).detach().numpy()
        
        if mask[5]:
            output = self.resnet_mlp(output, self.layer_fc_weights[0].transpose(), self.layer_fc_weights[1], act_fn=None, ncodebooks=self.n_subspace[5], ncentroids=self.k_cluster[5])
        else:
            output = np.dot(output, self.layer_fc_weights[0].transpose()) + self.layer_fc_weights[1]
            self.mm_res.append(output)
        output = self.sigmoid(torch.tensor(output)).detach().numpy()
    ...
    def eval_amm(self, input_data, mask):
        ...
        if mask[4]:
            layer5 = self.eval_block_amm(layer4, self.layer5_weights, self.layer5_bn_buffers, stride=2, n=self.n_subspace[4], k=self.k_cluster[4])
        else:
            layer5 = self.forward_block_exact(layer4, self.layer5_weights, self.layer5_bn_buffers, stride=2)
            
        output = self.avg_pool(torch.tensor(layer5)).view(input_data.shape[0], -1).detach().numpy()
        if mask[5]:
            output = self.resnet_mlp(output, self.layer_fc_weights[0].transpose(), self.layer_fc_weights[1], act_fn=None, ncodebooks=self.n_subspace[5], ncentroids=self.k_cluster[5], est=self.amm_est_queue.pop(0))
        else:
            output = np.dot(output, self.layer_fc_weights[0].transpose()) + self.layer_fc_weights[1]
            self.mm_res.append(output)
        output = self.sigmoid(torch.tensor(output)).detach().numpy()
