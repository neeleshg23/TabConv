...
def extract_weights_from_nin(state_dict):
    ...
    for i, block_key in enumerate(['block1', 'block2', 'block3']):
        for j in [0, 2, 4]:
            ...
            weights[f'{layer_key}_weights'] = cp.asarray(state_dict[f'{layer_key}.weight'].detach().numpy())
            biases[f'{layer_key}_bias'] = cp.asarray(state_dict[f'{layer_key}.bias'].detach().numpy())
